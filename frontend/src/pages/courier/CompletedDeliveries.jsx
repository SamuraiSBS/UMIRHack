...
export default function CompletedDeliveries() {
  ...
  useEffect(() => {
    api.get('/courier/orders')
      .then(r => setOrders(r.data.filter(o => o.status === 'DONE')))
  ...
  return (
    <div className="page" style={{ maxWidth: '560px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        {orders.map(o => (
          <div key={o.id} className="card" style={{ opacity: 0.9 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                <p style={{ fontWeight: 600, fontSize: '14px' }}>{o.business.name}</p>
                ...
                <p className="text-sm text-gray" style={{ marginTop: '4px' }}>
                  {new Date(o.createdAt).toLocaleDateString('ru', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}
                </p>
              </div>
              <div style={{ textAlign: 'right', flexShrink: 0, marginLeft: '12px' }}>
                {o.deliveryFee != null && (
                  <p style={{ fontWeight: 700, fontSize: '16px', color: '#16a34a' }}>+{o.deliveryFee.toFixed(0)} ₽</p>
                )}
                <p style={{ fontSize: '12px', color: '#6b7280' }}>заказ {o.totalPrice.toFixed(0)} ₽</p>
              </div>
            </div>
          </div>
        ))}
